...
namespace AppCitas.UnitTests.Tests
{
    public class LikesControllerTests
    {
        ...
        [Theory]
        [InlineData("NotFound", "wagner", "Pa$$w0rd","Photo")]
        public async Task AddLike_ShouldBeNotFound(string statusCode, string username, string password,string userLiked)
        {
            // Arrange
        ...
        [Theory]
        [InlineData("BadRequest", "wagner", "Pa$$w0rd", "wagner")]
        public async Task AddLike_ShouldBeBadRequest(string statusCode, string username, string password, string userLiked)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
        ...
        [Theory]
        [InlineData("OK", "wagner", "Pa$$w0rd", "esmeralda")]
        public async Task AddLike_ShouldBeOK(string statusCode, string username, string password, string userLiked)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
        ...
        [Theory]
        [InlineData("BadRequest", "wagner", "Pa$$w0rd", "esmeralda")]
        public async Task AddLike_ShouldBeBadRequest2(string statusCode, string username, string password, string userLiked)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
        ...
        public async Task GetUserLikes_ShouldBeOK(string statusCode, string username, string password)
        {
            // Arrange
            var user = await LoginHelper.LoginUser(username, password);
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", user.Token);
